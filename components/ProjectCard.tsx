...
const ProjectCard = ({title,description,tags,imageUrl}:projectProp) => {
    const ref = useRef<HTMLDivElement>(null)
   const {scrollYProgress}= useScroll({
        target:ref,
        offset:['0 1','1.13 1']
    })
    // use this to adjust how the scrollYprograss is used
    const scaleProgress = useTransform(scrollYProgress,[0, 1],[0.8,1])
  return (
    <motion.div ref={ref} className='mb-3 sm:mb-8 last:mb-0 group'
    style={{
        scale:scaleProgress,
